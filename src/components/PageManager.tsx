...
export const PageManager: React.FC<PageManagerProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded shadow-xl max-w-lg w-full max-h-[80vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h2 className="text-lg font-bold text-gray-900">管理页面</h2>
        ...
        <div className="p-5 overflow-y-auto flex-1">
          <div className="space-y-2 mb-6">
            {pages.map(page => {
              ...
              return (
                <div key={page.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded">
                  <Icon className="w-5 h-5 text-gray-500" />
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-900 text-sm">{page.title}</p>
                    <p className="text-sm text-text-light truncate">{page.description || '无描述'}</p>
                  </div>
                  <button
          ...
          <div className="border-t border-border pt-4">
            <h3 className="text-sm font-bold text-gray-700 mb-3">新增页面</h3>
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full py-2 bg-primary text-white rounded flex items-center justify-center gap-2
                         disabled:opacity-50 hover:bg-blue-800 transition-colors"
              >
      ...
      {deletingId && (
        <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded shadow-xl max-w-sm w-full p-5">
            <h3 className="text-lg font-bold text-gray-900 mb-2">确认删除页面？</h3>
